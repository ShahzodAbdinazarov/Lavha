...
> - **4-bosqich (klient control plane)** — mobil `3c7c3a3`: `SvipeMessageSync` (k-anon peer probe, `loadMyMode`/`setMyMode`/`deleteMyArchive`, 503→serverDisabled), `SvipeMessageSyncSettingsActivity` (3 rejim radio + off reciprocity confirm + serverdan o'chirish), `PrivacySettingsActivity` qatori, `ChatActivity` banner (mustaqil top-panel child, priority 9, `updateTopPanel`ga tegmaydi; 1:1 + peer Svipe + arxivlangan o'chirish + rejim tanlanmagan → ko'rinadi). Strings en/uz/ru, `SvipeMessageSyncTest`. **Backend flag default OFF** → peers/check 503, banner prod'da uxlab turadi.
> - **5-bosqich (yuklash — data plane)** — backend `92a889f`: `msg_crypto` AES-256-GCM (klient plaintext yuboradi, server at-rest muhrlaydi; kalit DB'дан tashqarida; kalitsiz=ochiq), `POST /observed` (with_partner faqat **ikkala rozi bo'lsa** qabul qiladi, self_only faqat o'ziniki; enumeration brake), `/upload-url` (muqova avatar bucket'ida `msg/` prefiks — yangi bucket kerak emas; cap+kunlik kvota), `/commit` (first-writer-wins), repo self funksiyalari, `cryptography>=43`. 12 unit + 4 DB integration (text ENCRYPTED at-rest + dedup, pending, self_only, media cover). Mobil `4cba8c6`: `SvipeMessageSync.syncDialogOnOpen` (chat ochilганда arxivни stage→observe→missing'ni yuklaydi = **backfill**; `mergeKey=sha256(author‖date‖contentHash)` ikki tomonда bir xil; serialize→base64; muqova only; bittalab, per-visit cap; per-dialog guard), `ChatActivity.onResume` trigger, `SvipeMessageSyncTest` merge-key.
> - **6-bosqich (o'qish + merge)** — backend `2d00440`: `GET /pair?peer_tg_id` (decide_pair_access gate — ikkala with_partner + opt-out yo'q; server sealни ochib base64 + presigned cover qaytaradi; daily fetch brake), `GET /self` (o'z backup'ini tiklash), ochib bo'lmagan qator o'tkazib yuboriladi. 3 unit + 4 DB integration (peer DECRYPTED o'qiydi, begona 403, opt-out 403, self). Mobil `1350456`: `SvipeMessageSync.fetch` (GET /pair|/self → deserialize → cover download → lokal insert), `MessagesStorage.svipeInsertSyncedMessage` (synthetic NEGATIVE mid `syntheticMid(mergeKey)` — deterministik/idempotent, manfiy → inline merge'ga tushmaydi, faqat log), 30s cooldown, `NotificationCenter.svipeMessageArchiveUpdated` → `SvipeDeletedLogActivity` observer reload.
> - **FEATURE FUNKSIONAL TO'LIQ (1–6). QOLGAN:** (a) owner emulyator E2E (ToS + rejim ekrani + banner + upload/read oqimi vizual ko'rilmagan — compile + pure + DB integration + string parity yashil); (b) 7-bosqich retention sweep + prod deploy (Play data-safety, `msg_sync_enabled=true`, `msg_sync_encryption_key`); (c) backend flag default OFF — prod'да hammasi uxlab turadi.
